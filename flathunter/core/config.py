...
from flathunter.core.exceptions import ConfigException
...
class Env:
    ...
    # Captcha setup
    TWOCAPTCHA_KEY = _read_env("2CAPTCHA_KEY")
    IMAGETYPERZ_TOKEN = _read_env("IMAGETYPERZ_TOKEN")
    CAPMONSTER_KEY = _read_env("CAPMONSTER_KEY")
    HEADLESS_BROWSER = _read_env("HEADLESS_BROWSER")
    IS24_COOKIE = _read_env("IS24_COOKIE")

    # Generic Config
    TARGET_URLS = _read_env("TARGET_URLS")
    DATABASE_LOCATION = _read_env("DATABASE_LOCATION")
    GOOGLE_CLOUD_PROJECT_ID = _read_env(
        "GOOGLE_CLOUD_PROJECT_ID")
    VERBOSE_LOG = _read_env("VERBOSE_LOG")
    LOOP_PERIOD_SECONDS = _read_env(
        "LOOP_PERIOD_SECONDS")
    RANDOM_JITTER_ENABLED = _read_env("RANDOM_JITTER_ENABLED")
    LOOP_PAUSE_FROM = _read_env("LOOP_PAUSE_FROM")
    LOOP_PAUSE_TILL = _read_env("LOOP_PAUSE_TILL")
    MESSAGE_FORMAT = _read_env("MESSAGE_FORMAT")

    # Website setup
    WEBSITE_SESSION_KEY = _read_env(
        "WEBSITE_SESSION_KEY")
    WEBSITE_DOMAIN = _read_env("WEBSITE_DOMAIN")
    WEBSITE_BOT_NAME = _read_env("WEBSITE_BOT_NAME")

    # Notification setup
    NOTIFIERS = _read_env("NOTIFIERS")
    TELEGRAM_BOT_TOKEN = _read_env("TELEGRAM_BOT_TOKEN")
    TELEGRAM_BOT_NOTIFY_WITH_IMAGES = \
        _read_env("TELEGRAM_BOT_NOTIFY_WITH_IMAGES")
    TELEGRAM_RECEIVER_IDS = _read_env(
        "TELEGRAM_RECEIVER_IDS")
    MATTERMOST_WEBHOOK_URL = _read_env(
        "MATTERMOST_WEBHOOK_URL")
    SLACK_WEBHOOK_URL = _read_env("SLACK_WEBHOOK_URL")
    APPRISE_NOTIFY_WITH_IMAGES = _read_env(
        "APPRISE_NOTIFY_WITH_IMAGES")
    APPRISE_IMAGE_LIMIT = _read_env(
        "APPRISE_IMAGE_LIMIT")

    # Filters
    FILTER_EXCLUDED_TITLES = _read_env(
        "FILTER_EXCLUDED_TITLES")
    FILTER_MIN_PRICE = _read_env("FILTER_MIN_PRICE")
    FILTER_MAX_PRICE = _read_env("FILTER_MAX_PRICE")
    FILTER_MIN_SIZE = _read_env("FILTER_MIN_SIZE")
    FILTER_MAX_SIZE = _read_env("FILTER_MAX_SIZE")
    FILTER_MIN_ROOMS = _read_env("FILTER_MIN_ROOMS")
    FILTER_MAX_ROOMS = _read_env("FILTER_MAX_ROOMS")
    FILTER_MAX_PRICE_PER_SQUARE = _read_env(
        "FILTER_MAX_PRICE_PER_SQUARE")
...
class CaptchaEnvironmentConfig(YamlConfig):
    ...
    def _get_imagetyperz_token(self):
        return Env.IMAGETYPERZ_TOKEN() or super()._get_imagetyperz_token()  # pylint: disable=no-member

    def get_twocaptcha_key(self) -> str:
        """Return the currently configured 2captcha API key"""
        return Env.TWOCAPTCHA_KEY() or super().get_twocaptcha_key()  # pylint: disable=no-member

    def get_capmonster_key(self) -> str:
        """Return the currently configured Capmonster API key"""
        return Env.CAPMONSTER_KEY() or super().get_capmonster_key()

    def captcha_driver_arguments(self):
        """The list of driver arguments for Selenium / Webdriver"""
        if Env.HEADLESS_BROWSER() is not None:
            return [
                "--no-sandbox",
                "--headless",
                "--disable-gpu",
...
class Config(CaptchaEnvironmentConfig):  # pylint: disable=too-many-public-methods
    ...
    def __init__(self, filename=None):
        if filename is None and Env.TARGET_URLS() is None:
            raise ConfigException(
                "Config file loaction must be specified, or TARGET_URLS must be set")
